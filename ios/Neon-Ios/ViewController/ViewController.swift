...
//        }
    ...
    private func checkAllValidationOnSubmitButtonAction(message: String, arrayTagModel: [ImageTagModel], isMendatory: Bool) -> Bool {
        for tagModel in arrayTagModel {
            let arr   = NeonImagesHandler.singleonInstance.imagesCollection.filter {
                $0.fileTag?.tagName == tagModel.tagName
            }
            if tagModel.noOfPhotos ?? 0 >  arr.count {
                
                var messageUpdate = message
                if isMendatory {
                    messageUpdate = message + "\(tagModel.tagName ?? "")"
                }
                if isMendatory == true {
    ...
    private func makeJsonToSendDataAfterAllChecks() {
        ...
        let mendatoryArray = NeonImagesHandler.singleonInstance.imageTagArray.filter { $0.mandatory == true}
        ...
        //check images are filled
        let nonMendatoryArray = NeonImagesHandler.singleonInstance.imageTagArray.filter { $0.mandatory == false}
...
extension ViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    internal func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        DispatchQueue.main.async {
            if let filePath = fileinfo.filePath, filePath != "" {
        ...
        guard let imageTageModel = fileinfo.fileTag, imageTageModel.isSelected == true
            else {
                cell.tagName.text = "Select Tag"
                return cell
        }
        cell.tagName.text = imageTageModel.tagName
        return cell
    }
                                            ...
                                            if let info = info {
                                                if info.keys.contains(NSString(string: "PHImageFileURLKey")) {
                                                    if let filePath = info[NSString(string: "PHImageFileURLKey")] as? URL {
                                                        ...
                                                        //Set File path
                                                        fileInfo.filePath = absoluteString.replacingOccurrences(of: "file:///", with: "")
                                                        
                                                        //Set File Name
                                                        fileInfo.fileName = filePath.lastPathComponent

                                                        //Set Source
                                                        fileInfo.source = SOURCE.PHONE_GALLERY
                                                        
                                                        //Set Date TIme
                                                        let dateFormatter = DateFormatter()
                                                        dateFormatter.dateFormat = "yyyy-MM-dd hh:mm:ss"
                                                        fileInfo.dateTimeTaken = self.convertDateToString(date: asset.creationDate!)
                                                        
                                                        //Set Time Stamp
                                                        fileInfo.timestamp = self.convertDateToString(date: asset.creationDate!)
                                                        
                                                        //Set Latitude and Longitude
                                                        
                                                        if let locationInfo = asset.location {
                                                            fileInfo.latitude = String(locationInfo.coordinate.latitude)
                                                            fileInfo.longitude = String(locationInfo.coordinate.longitude)
                                                        }
