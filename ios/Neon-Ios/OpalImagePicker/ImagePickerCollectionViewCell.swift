...
protocol ImagePickerCollectionViewCellprotocol: class {
...
class ImagePickerCollectionViewCell: UICollectionViewCell {
    ...
    weak var delegate: ImagePickerCollectionViewCellprotocol?
    ...
    private func loadPhotoAssetIfNeeded() {
        ...
        imageRequestID = manager.requestImageData(for: asset, options: options,
                                 resultHandler: { [weak self]  (imagedata, dataUTI, orientation, info) in
                                    guard self?.indexPath?.item == indexPath.item else {
                                        return
                                    }
                                    ...
                                    //Received Image
                                        ...
                                        if info.keys.contains(NSString(string: "PHImageFileURLKey")) {
                                            if let filePath = info[NSString(string: "PHImageFileURLKey")] as? URL {
                                                ...
                                                if let image = NeonImagesHandler.singleonInstance.imageLocalPathSaved.object(forKey: filePath.absoluteString.replacingOccurrences(of: "file:///", with: "") as NSString) {
                                                    self?.imageView.image = image
                                                } else {
                                                    DispatchQueue.main.async {
                                                        self?.imageView.image = UIImage(contentsOfFile: filePath.absoluteString.replacingOccurrences(of: "file:///", with: ""))
                                                        NeonImagesHandler.singleonInstance.imageLocalPathSaved.setObject((self?.imageView.image)!, forKey: filePath.absoluteString.replacingOccurrences(of: "file:///", with: "") as NSString)
                                                    }
                                                }
                                                for item in NeonImagesHandler.singleonInstance.imagesCollection {
                                                   if filePath.absoluteString.replacingOccurrences(of: "file:///", with: "") == item.filePath {
                                                self?.delegate?.updateCell(indexPath: indexPath, asset: asset)
...
//        })
    ...
    func addOverlay(_ animated: Bool) {
