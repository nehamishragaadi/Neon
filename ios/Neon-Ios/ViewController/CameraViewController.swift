...
class CameraViewController: UIViewController ,UIImagePickerControllerDelegate,UINavigationControllerDelegate  {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.tagTextView.text = NeonImagesHandler.singleonInstance.imageTagArray[0].tagName
    ...
    @IBAction func previousButtonTapped(_ sender: Any) {
        if tagIndex > 0 {
            tagIndex = tagIndex - 1;
            self.tagTextView.text = NeonImagesHandler.singleonInstance.imageTagArray[tagIndex].tagName
        }
        self.checkIndex()
    }
...
extension CameraViewController: AVCapturePhotoCaptureDelegate {
    func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        if tagModel.noOfPhotos ?? 0 > arr.count || NeonImagesHandler.singleonInstance.imageTagArray[self.tagIndex].noOfPhotos == 0  {
            ...
            SaveImageInGallery.sharedInstance.saveImage(image: image!, albumName: "TestingNeon111") { (filePath, error) in
                ...
                fileInfo.source = SOURCE.PHONE_CAMERA
                tagModel.isSelected = true
                fileInfo.fileTag?.tagId = tagModel.tagId
                ...
                if self.tagIndex < NeonImagesHandler.singleonInstance.imageTagArray.count - 1 || tagModel.noOfPhotos ?? 0 > arr2.count{
                    
                    if tagModel.noOfPhotos == arr2.count {
                        self.tagIndex = self.tagIndex + 1;
                    }
                    let tagModel: ImageTagModel = NeonImagesHandler.singleonInstance.imageTagArray[self.tagIndex]
                    self.tagTextView.text = tagModel.tagName
                    self.checkIndex()
                } else {
                    DispatchQueue.main.async {
            ...
            if tagModel.noOfPhotos == arr.count {
