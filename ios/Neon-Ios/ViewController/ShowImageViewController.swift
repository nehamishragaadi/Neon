...
//            fileInfo.setFilePath(filePath: filePath!)
    ...
    @IBAction func showImageTagControllerAction(_ sender: Any) {
        if imageCollection.indexPathsForVisibleItems.count > 0 {
            ...
            viewController.completionHandler = {                self.selectTagButton.setTitle(NeonImagesHandler.singleonInstance.imagesCollection[self.imageCollection.indexPathsForVisibleItems.first!.row].fileTag?.tagName, for: .normal)
            ...
            for item in NeonImagesHandler.singleonInstance.imageTagArray {
                for fileInfo in NeonImagesHandler.singleonInstance.imagesCollection {
                    if fileInfo.fileTag?.tagName != item.tagName{
...
extension ShowImageViewController : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    internal func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        DispatchQueue.main.async {
            if let image = NeonImagesHandler.singleonInstance.imageLocalPathSaved.object(forKey: fileInfo.filePath as NSString) {
                cell.imageView.image = image
            } else {
                DispatchQueue.main.async {
                    cell.imageView.image = UIImage(contentsOfFile: fileInfo.filePath)
                    NeonImagesHandler.singleonInstance.imageLocalPathSaved.setObject(cell.imageView.image!, forKey: fileInfo.filePath as NSString)
                }}
        }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let fileInfo = NeonImagesHandler.singleonInstance.imagesCollection[indexPath.row]
        if fileInfo.fileTag?.isSelected == true,
        let tagName = fileInfo.fileTag?.tagName {
