...
open class OpalImagePickerRootViewController: UIViewController, ImagePickerCollectionViewCellprotocol {
    ...
    private func setupTabs() {
        ...
        NSLayoutConstraint.activate([
            ...
            toolbar.constraintEqualTo(with: view as Any, attribute: .left),
            toolbar.constraintEqualTo(with: view as Any, attribute: .right)
            ])
    }
    ...
    open override func viewDidLoad() {
        super.viewDidLoad()
        setup()
        print("selected assest count",self.selectedAsset.count)
...
//        delegate?.imagePicker?(imagePicker, didFinishPickingImages: savedImages)
    ...
    private func set(image: UIImage?, indexPath: IndexPath, isExternal: Bool) {
       self.imageDeletion(indexpath: indexPath)
    ...
    func updateCell(indexPath: IndexPath, asset: PHAsset) {
        guard let cell = self.collectionView?.cellForItem(at: indexPath) as? ImagePickerCollectionViewCell,
            let image = cell.imageView.image else { return }
        set(image: image, indexPath: indexPath, isExternal: false)
        self.collectionView?.selectItem(at: indexPath, animated: true, scrollPosition: .centeredHorizontally)
        cell.addOverlay(true)
    }
    ...
    private func update(isSelected: Bool, isExternal: Bool, for indexPath: IndexPath) {
        ...
        selectedIndexPaths = selectedIndexPaths.removeDuplicates()
    }
...
extension OpalImagePickerRootViewController: UICollectionViewDelegate {
    ...
    public func imageDeletion(indexpath: IndexPath){
        let assetDeselcted = photoAssets[indexpath.row]
        var imageUrl_deselect: URL?
        let options = PHImageRequestOptions()
        options.deliveryMode = .opportunistic
        options.isSynchronous = true
        options.isNetworkAccessAllowed = true
        PHImageManager.default().requestImageData(for: assetDeselcted, options: options, resultHandler: { imageData, dataUTI, orientation, info in
            if let info = info {
                print("info = \(info)")
            }
            if info?["PHImageFileURLKey"] != nil {
                
                imageUrl_deselect = info?["PHImageFileURLKey"] as? URL
            }
        })
        
        print("selectedAsset count previous",self.selectedAsset.count)
        let selectedAssetLoop: [PHAsset] = self.selectedAsset
        if selectedAssetLoop.count > 0 {
            for i in 0...selectedAssetLoop.count-1{
                let asset = selectedAssetLoop[i]
                PHImageManager.default().requestImage(for: asset , targetSize: PHImageManagerMaximumSize, contentMode: PHImageContentMode.default, options: options, resultHandler: { (pickedImage, info) in
                    
                    if info?["PHImageFileURLKey"] != nil {
                        let path = info?["PHImageFileURLKey"] as? URL
                        if imageUrl_deselect == path {
                            self.selectedAsset.remove(at: i)
                            print("selectedAsset count after",self.selectedAsset.count)
